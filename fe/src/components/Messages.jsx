...
const Messages = ({ channelId, state, messageId, ...props }) => {
  const { socket, setSelectedFiles } = useContext(SocketContext);
  const { users } = useUser();
  const { auth } = useAuth();
  const ref = useRef();
  ...
  const onUpdateMessage = (status, data) => {
    if (status == STATUS.ON) {
      console.log(data);
      setMessages((prev) =>
        prev.map((message) => (message._id == data._id ? data : message))
      );
    }
  };
    ...
    if (data.channelId == channelId && data.messageId == messageId) {
      ...
      setTypingList((prev) => {
        if (prev.some((user) => user == data.user)) {
          ...
          timerRef.current[data.user] = setTimeout(removeUser, 1000);
          return prev;
        }
        timerRef.current[data.user] = setTimeout(removeUser, 1000);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      socket.removeListener(
        `${REQUEST.MESSAGE}_${METHOD.DELETE}`,
        onDeleteMessage
      );
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (channelId) {
      socket.emit(`${REQUEST.MESSAGE}_${METHOD.READ}`, {
        channel: channelId,
        parent: messageId,
      });
    }
  ...
  return (
    <VStack align="stretch" {...props}>
      ...
      <Box h={3} px={4}>
        {typingList.length > 0 && (
          <Text fontSize="sm" hidden={typingList.includes(auth.id) && true}>
            {typingList
              .map((userId) => {
