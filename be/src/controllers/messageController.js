...
exports.create = async (socket, data) => {
    try {
        ...
        const channel = await channelService.readOne(data.channel);
        const createdMessage = await messageService.readOne(message._id)
        sendToUsers(socket.socketList, channel.members, `${REQUEST.MESSAGE}_${METHOD.CREATE}`, STATUS.ON, createdMessage);
        if (data.parent) {
        ...
        socket.emit(`${REQUEST.MESSAGE}_${METHOD.CREATE}`, STATUS.SUCCESS, createdMessage);
    } catch (err) {
...
exports.updatePin = async (socket, data) => {
    try {
        const prevMsg = await messageService.readOne(data.id);
        const message = await messageService.update('', data.id, { isPined: !prevMsg.isPined });
        const channel = await channelService.readOne(message.channel);
