...
exports.read = async (data) => {
    const messages = await Message.find(data).populate('sender').populate('mentions').populate('files');
    if (data.parent == null) {
...
exports.readOne = async (id) => {
    const message = await Message.findById(id).populate('sender').populate('mentions').populate('files');
    if (!message)
        throw new Error('Not found message');
    const childCount = await Message.find({ parent: id }).populate('sender');
    message.childCount = childCount;
    return message;
}
