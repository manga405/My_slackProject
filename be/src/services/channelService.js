...
exports.read = (userId) => {
    return Channel.find({ members: { $in: [userId] } }).populate('members').populate('invited').populate('creator');
}

exports.readAll = () => {
    return Channel.find().populate('members').populate('invited').populate('creator');
}

exports.readOne = async (id) => {
    const channel = await Channel.findById(id).populate('members').populate('invited').populate('creator');
    if (!channel)
